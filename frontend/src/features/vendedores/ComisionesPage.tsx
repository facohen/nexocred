...
export function ComisionesPage({ vendedorId }: { vendedorId: string }) {
  ...
  if (q.isLoading) return <p className="p-4 text-sm text-foreground/60">Cargando comisiones…</p>;
  if (q.isError) return <p role="alert" className="p-4 text-sm text-red-700">No se pudieron cargar las comisiones.</p>;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {totales.map(({ estado, total }) => (
          <Card key={estado}>
            <div className="text-xs capitalize text-foreground/60">{estado}</div>
          ...
          <tbody>
            {comisiones.map((c) => (
              <tr key={c.id} className="border-b border-border last:border-0">
                ...
                <td className="py-1"><Badge tone={c.estado === "clawback" ? "danger" : "default"}>{c.estado}</Badge></td>
                <td className="py-1">{c.tipo}</td>
                <td className="py-1 text-right"><MoneyText value={c.monto} /></td>
              </tr>
            ))}
          </tbody>
        </table>
