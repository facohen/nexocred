...
/**
 * Formulario reutilizable para registrar un pago. Encapsula toda la lógica de
 * negocio (mismo hook useRegistrarPago, misma idempotency-key, mismo
 * EntityCombobox y preview del waterfall). Lo usan tanto RegistrarPagoPage
 * (inline en la ruta /pagos) como RegistrarPagoSheet (panel contextual).
 */
export function PagoForm({ prestamoId: prestamoIdProp }: { prestamoId?: string }) {
  // IDs antes hardcodeados → ahora seleccionados por inbox (EntityCombobox).
  const [prestamoId, setPrestamoId] = useState<string | null>(prestamoIdProp ?? null);
