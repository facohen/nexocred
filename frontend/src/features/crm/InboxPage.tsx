...
export function InboxPage() {
  ...
  if (q.isLoading) return <p className="p-4 text-sm text-text-muted">Cargando tareas…</p>;
  if (q.isError) return <p role="alert" className="p-4 text-sm text-neg">No se pudieron cargar las tareas.</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      {tareas.length === 0 ? (
        <p className="text-sm text-text-muted">No hay tareas pendientes.</p>
      ) : (
            ...
            <li key={t.id}>
              <Card className="flex items-center justify-between">
                <div>
                  ...
                  {t.descripcion && <div className="text-xs text-text-muted">{t.descripcion}</div>}
